// =============================================================================
// DOUKÈ Compta Pro - Gestionnaire de données avec isolation v3.1
// =============================================================================
...
class DataManager {
    constructor(securityManager) {
        ...
        console.log('💾 DataManager avec isolation initialisé');
    }

    // Configuration des validateurs de données
    setupValidators() {
    ...
    // Obtenir toutes les données d'une entreprise avec vérification d'accès
    getCompanyData(companyId, userId = null) {
        ...
        if (!this.security.hasAccessToCompany(currentUserId, companyId)) {
            throw new SecurityError(`🚫 Accès refusé à l'entreprise ${companyId}`);
        }
    ...
    // Obtenir les écritures d'une entreprise
    getCompanyEntries(companyId, userId = null) {
        const currentUserId = userId || (window.app.currentUser ? window.app.currentUser.id : null);
        
        if (!this.security.hasAccessToCompany(currentUserId, companyId)) {
            throw new SecurityError(`🚫 Accès refusé aux écritures de l'entreprise ${companyId}`);
        }

        const entries = window.app.entries.filter(entry => entry.companyId === companyId);
        
        // Filtrage supplémentaire selon le profil
        const user = this.security.getCurrentUser(currentUserId);
        if (user && user.profile === 'caissier') {
            // Les caissiers ne voient que les écritures de caisse
    ...
    getCompanyAccounts(companyId, userId = null) {
        ...
        if (!this.security.hasAccessToCompany(currentUserId, companyId)) {
            throw new SecurityError(`🚫 Accès refusé au plan comptable de l'entreprise ${companyId}`);
        }

        // Le plan comptable SYSCOHADA est global mais peut être filtré par entreprise
        return window.app.accounts.filter(account => {
            // Logique de filtrage si nécessaire (comptes spécifiques à l'entreprise)
    ...
    getCompanyCashRegisters(companyId, userId = null) {
        ...
        if (!this.security.hasAccessToCompany(currentUserId, companyId)) {
            throw new SecurityError(`🚫 Accès refusé aux caisses de l'entreprise ${companyId}`);
        }
    ...
    getCompanyUsers(companyId, userId = null) {
        ...
        if (!this.security.hasAccessToCompany(currentUserId, companyId)) {
            throw new SecurityError(`🚫 Accès refusé aux utilisateurs de l'entreprise ${companyId}`);
        }
    ...
    // Créer une nouvelle entrée avec validation
    createEntry(entryData, companyId) {
        // Vérifications de sécurité
        if (!this.security.hasAccessToCompany(window.app.currentUser.id, companyId)) {
            throw new SecurityError('Accès refusé pour créer une écriture dans cette entreprise');
        }
    ...
    updateEntry(entryId, updateData) {
        ...
        if (!this.security.hasAccessToCompany(window.app.currentUser.id, entry.companyId)) {
            throw new SecurityError('Accès refusé pour modifier cette écriture');
        }
    ...
    deleteEntry(entryId) {
        ...
        // Vérifications de sécurité
        if (!this.security.hasAccessToCompany(window.app.currentUser.id, entry.companyId)) {
            throw new SecurityError('Accès refusé pour supprimer cette écriture');
        }

        if (entry.status === 'Validé' && window.app.currentUser.profile !== 'admin') {
    ...
    validateEntry(entryData) {
        ...
        if (!entryData.lines || !Array.isArray(entryData.lines) || entryData.lines.length < 2) {
            throw new Error('Une écriture doit avoir au moins 2 lignes');
        }
        ...
        if (Math.abs(totalDebit - totalCredit) > 0.01) {
            throw new Error('L\'écriture doit être équilibrée (Total Débit = Total Crédit)');
        }
    ...
    validateCashRegister(cashData) {
        ...
        if (cashData.maxLimit && cashData.maxLimit < 0) {
            throw new Error('La limite maximale ne peut pas être négative');
        }
    ...
    // Système de listeners pour les changements de données
    ...
    // Obtenir les statistiques de données
    ...
    clearCache() {
        ...
        console.log('💾 Cache des données nettoyé');
    }

    // Sauvegarde des données (simulation)
    async saveData() {
        try {
            // Dans un environnement réel, ceci sauvegarderait dans une base de données
