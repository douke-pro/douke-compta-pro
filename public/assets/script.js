...
async function handleLogin(event) {
  ...
  try {
    ...
    // 6.bis PERSISTENCE (correction de la perte de session au refresh)
...
function handleLogout() {
  ...
  // 1.bis Effacer la persistence locale
...
function switchCompany(companyId) {
  ...
  // Mettre à jour la persistence pour cohérence au refresh
...
function checkAuthAndRender() {
  // Si un token est en session (ou sauvegardé), rendre le dashboard
  const savedToken = localStorage.getItem('douke_token');
  if (savedToken) {
    // Restaurer le contexte minimal pour affichage direct sans re-login
...
function renderFinancialStatementsModule(contentArea) {
  ...
  contentArea.innerHTML = `
    ...
    <div id="financial-reports-area" class="mt-6 space-y-4"></div>
  `;
  ...
  contentArea.innerHTML = `
    ...
    <div class="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-2xl shadow-XL border border-gray-100 dark:border-gray-700 fade-in">
      <form id="quick-entry-form" onsubmit="handleQuickEntrySubmit(event)">
        ...
        <div id="entry-lines-container" class="space-y-4"></div>
