...
window.handleCompanyChange = async function (newCompanyId) { 
    ...
    if (newCompany) {
        ...
        localStorage.setItem('douke_active_company_id', String(newId));
...
function printModalContent(containerId, title) {
    ...
    var css = [
        ...
        '.space-y-4 > * + * { margin-top: 10px; } .space-y-3 > * + * { margin-top: 6px; }',
        ...
        '.p-4 { padding: 8px; } .p-3 { padding: 6px; } .px-3 { padding: 0 6px; } .py-2 { padding: 4px 0; }',
        '.pb-4 { padding-bottom: 10px; } .pt-4 { padding-top: 10px; }',
        ...
        '.rounded-xl { border-radius: 4px; } .overflow-hidden { overflow: hidden; }',
        '.w-full { width: 100%; } .min-w-full { min-width: 100%; }',
        '.bg-gray-50 { background: #f8f9fa; } .bg-white { background: #fff; }',
        '.text-xs { font-size: 7.5pt; } .text-sm { font-size: 8.5pt; } .text-base { font-size: 10pt; }',
        '.text-lg { font-size: 12pt; } .text-xl { font-size: 14pt; } .text-2xl { font-size: 16pt; }',
...
function isAdminOrCollab() {
    const role = (appState.user?.role || appState.user?.profile || '').toLowerCase();
...
// =============================================================================
// BLOC 1 — generateManualEntryFormHTML
// Remplacez entièrement votre fonction existante par celle-ci
// =============================================================================
    ...
    return `
        <div class="max-w-6xl mx-auto">
            <div class="flex justify-between items-center mb-8">
                ...
                <div id="period-badge"
                    class="px-4 py-2 rounded-full text-xs font-bold border shadow-sm flex items-center gap-2
                    ${fiscalYear
                    ...
                    ${periodLabel}
                    ${!fiscalYear ? `
                        <button onclick="openFiscalYearModal()"
                            class="ml-2 underline font-bold text-warning hover:text-warning/80">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pb-6 border-b border-gray-100">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                max="${maxDate}"
                                required
                                class="w-full rounded-xl border-gray-200 p-3 shadow-sm focus:ring-2 focus:ring-primary font-bold">
                            <p class="text-xs text-gray-400 mt-1">
                                Période : ${new Date(minDate).toLocaleDateString('fr-FR')}
                        ...
                        <div>
                            ...
                            <select id="journal-code"
                                class="w-full rounded-xl border-gray-200 p-3 shadow-sm focus:ring-2 focus:ring-primary"
                                required>
                        ...
                        <div>
                            ...
                            <input type="text" id="narration"
                                class="w-full rounded-xl border-gray-200 p-3 shadow-sm focus:ring-2 focus:ring-primary"
                                placeholder="Ex: FACTURE-2022-XYZ"
                                required>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row items-center justify-between pt-6 border-t border-gray-100 gap-4">
                        ...
                        <div class="flex flex-col items-end">
                            ...
                            <button type="submit" id="submit-btn"
                                class="mt-2 bg-primary text-white font-black py-4 px-10 rounded-2xl shadow-xl hover:scale-105 active:scale-95 disabled:opacity-30 transition-all uppercase tracking-widest"
                                disabled>
    ...
    if (token) {
        ...
        try {
            ...
            if (response && response.data) {
                ...
                // ✅ Restaurer l'entreprise réellement active avant le refresh (si valide)
                const savedCompanyId = parseInt(localStorage.getItem('douke_active_company_id'));
                if (savedCompanyId && appState.user.companiesList.some(c => c.id === savedCompanyId)) {
                    appState.currentCompanyId = savedCompanyId;
                    appState.user.selectedCompanyId = savedCompanyId;
                    const restoredCompany = appState.user.companiesList.find(c => c.id === savedCompanyId);
                    appState.currentCompanyName = restoredCompany ? restoredCompany.name : appState.currentCompanyName;
                    console.log(`🏢 [DOMContentLoaded] Entreprise restaurée depuis localStorage: ${savedCompanyId}`);
                }
            ...
            <form id="company-settings-form" onsubmit="window.handleSaveCompanySettings(event)" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="company-settings-rccm" value="${data.registration_number || ''}" 
                            ${!isAdmin ? 'disabled' : ''}
                            class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:border-gray-600 ${!isAdmin ? 'bg-gray-100' : ''}" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="tel" id="company-settings-phone" value="${data.phone || ''}" 
                            ${!isAdmin ? 'disabled' : ''}
                            class="w-full p-3 border rounded-xl dark:bg-gray-700 dark:border-gray-600 ${!isAdmin ? 'bg-gray-100' : ''}" 
...
window.handleSaveCompanySettings = async function(event) {
    ...
    const data = {
        ...
        phone: document.getElementById('company-settings-phone').value,
        address: document.getElementById('company-address').value,
        city: document.getElementById('company-city').value,
        tax_id: document.getElementById('company-nif').value,
        registration_number: document.getElementById('company-settings-rccm').value
    };
    ...
    try {
        ...
        // ✅ Mettre à jour le cache local pour que le formulaire reste cohérent
        // lors d'un changement d'onglet (sans attendre un rechargement complet)
        if (window.settingsData?.company) {
            window.settingsData.company.name = data.name;
            window.settingsData.company.email = data.email;
            window.settingsData.company.phone = data.phone;
            window.settingsData.company.address = data.address;
            window.settingsData.company.city = data.city;
            window.settingsData.company.tax_id = data.tax_id;
            window.settingsData.company.registration_number = data.registration_number;
        }
...
window.printContract = async function(employeeId) {
    try {
        ...
        const vars = {
            ...
            numero_piece:            emp.id_number || '{{numero_piece}}',
...
window.saveHREmployee = async function(employeeId) {
    const body = {
        ...
        date_naissance:   document.getElementById('emp-dob')?.value || null,
...
window.downloadPayslip = async function(payslipId) {
    try {
        ...
        // Calculs (valeurs reelles stockees lors de l'enregistrement de la fiche)
