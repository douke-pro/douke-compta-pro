...
// VERSION : OPTIMALE 4.0 - Alignée avec server.js + Architecture hybride V2.2/V3.0
// =================================================================================
 ...
 * @returns {object|null} - Contexte utilisateur ou null si échec
 */
async function handleLogin(email, password) {
    ...
    try {
        ...
        if (response.ok) {
            // Le serveur retourne directement l'objet utilisateur
            return {
 ...
 * @returns {object} - Contexte utilisateur simulé
 */
async function handleRegistration(payload) {
    // TEMPORAIRE: Simulation côté client en attendant endpoint serveur
...
async function fetchUserCompanies(context) {
    ...
    try {
        ...
        console.error('Erreur réseau lors de la récupération des entreprises:', error);
        return [];
    }
}
...
function initDashboard(context) {
    ...
    // Cacher les vues d'authentification
...
function updateNavigationMenu(role) {
    ...
    // Fonctionnalités métier (nécessitent un contexte entreprise)
...
async function renderDashboard(context) {
    // Admin/Collab sans contexte → Dashboard Global
    ...
    // Admin/Collab avec contexte → Dashboard spécifique entreprise
    if ((context.utilisateurRole === ROLES.ADMIN || context.utilisateurRole === ROLES.COLLABORATEUR) && context.entrepriseContextId) {
        return await renderCompanySpecificDashboard(context);
    }
    
    // User/Caissier → Dashboard mono-entreprise
...
// =================================================================================
// 6. PLACEHOLDERS FORMULAIRES (À COMPLÉTER SELON VOS BESOINS)
// =================================================================================
