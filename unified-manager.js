// =============================================================================
// DOUKÈ Compta Pro - Gestionnaire Unifié Principal v3.1
// =============================================================================
...
class UnifiedManager {
    constructor() {
        ...
        // Références vers les gestionnaires
        ...
        console.log('🚀 UnifiedManager v' + this.version + ' initialisé');
    }
    ...
    async initialize() {
        try {
            ...
            // 3. Initialiser la sécurité et l'authentification
            this.initializeSecurity();
            
            // 4. Initialiser les contrôleurs métier
    ...
    initializeData() {
        ...
        if (typeof initializeSampleData === 'function') {
            ...
            throw new Error('Module de données d\'exemple non disponible');
        }
    ...
    // Initialiser les contrôleurs métier
    initializeControllers() {
        ...
        if (typeof UsersController !== 'undefined') {
            ...
            console.warn('⚠️ UsersController non disponible');
        }
        ...
        if (typeof CompaniesController !== 'undefined') {
            ...
            console.warn('⚠️ CompaniesController non disponible');
        }
        
        // Contrôleur des écritures
        if (typeof EntriesController !== 'undefined') {
            ...
            console.warn('⚠️ EntriesController non disponible');
        }
        ...
        if (typeof AccountsController !== 'undefined') {
            ...
            console.warn('⚠️ AccountsController non disponible');
        }
        ...
        if (typeof CashController !== 'undefined') {
            ...
            console.warn('⚠️ CashController non disponible');
        }
        ...
        if (typeof ReportsController !== 'undefined') {
            ...
            console.warn('⚠️ ReportsController non disponible');
        }
    ...
    setupGlobalErrorHandling() {
        window.addEventListener('error', (event) => {
            ...
            this.notificationManager?.show('error', 'Erreur système', 'Une erreur inattendue s\'est produite');
        });
        
        window.addEventListener('unhandledrejection', (event) => {
            console.error('Promise rejetée:', event.reason);
            this.notificationManager?.show('error', 'Erreur système', 'Une erreur asynchrone s\'est produite');
        });
    }
    ...
    showLoginScreen() {
        if (this.authController) {
            ...
            throw new Error('AuthController non disponible pour afficher l\'écran de connexion');
        }
    }
    ...
    async login(email, password) {
        ...
        try {
            ...
            if (result.success) {
                this.showDashboard();
                this.notificationManager?.show('success', 'Connexion réussie', 
    ...
    logout() {
        if (this.authController) {
            ...
            this.showLoginScreen();
            this.notificationManager?.show('info', 'Déconnexion', 'Vous avez été déconnecté avec succès');
        }
    }
    ...
    showDashboard() {
        ...
        // Charger le contenu par défaut selon le profil
    ...
    generateMainInterface() {
        ...
        return `
            <!-- Interface principale DOUKÈ Compta Pro -->
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <button onclick="window.unifiedManager.themeManager?.toggleTheme()" 
                                        class="p-2 text-gray-600 dark:text-gray-400 hover:text-primary" title="Changer le thème">
    ...
    // Générer le sélecteur d'entreprise
            ...
            return `
                <div class="relative">
                    <button onclick="toggleCompanySelector()" 
                        ...
                        <span class="hidden md:inline">${currentCompany ? currentCompany.name : 'Sélectionner une entreprise'}</span>
    ...
    // Obtenir les éléments de menu selon le profil
    getMenuItemsForProfile(profile) {
        ...
        if (profile === 'admin') {
            return [
                ...
                { icon: 'fas fa-edit', label: 'Écritures', action: 'loadEntriesPage' },
            ...
            return [
                ...
                { icon: 'fas fa-print', label: 'État Caisse', action: 'loadReportsPage' }
            ];
        }
    ...
    selectCompany(companyId) {
        ...
        if (!this.security?.canAccessCompany(window.app.currentUser?.id, companyId)) {
            this.notificationManager?.show('error', 'Accès refusé', 'Vous n\'avez pas accès à cette entreprise');
            return;
        }
        
        window.app.currentCompanyId = companyId;
        this.notificationManager?.show('success', 'Entreprise sélectionnée', `${company.name} est maintenant active`);
    ...
    // Générer le contenu du tableau de bord
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            ...
                            <div class="text-sm text-gray-500">DOUKÈ Compta Pro v${this.version}</div>
                        </div>
                    </div>
                </div>
            ...
            stats.push(
                {
                    ...
                    description: 'Total des écritures'
                },
                {
                    label: 'En attente',
    ...
    getQuickActionsForProfile(profile) {
        ...
        if (window.app.currentCompanyId) {
            actions.push(
                { label: 'Nouvelle écriture', action: 'showNewEntryModal', icon: 'fas fa-plus', color: 'text-purple-500' },
    ...
    initializeInterfaceEvents() {
        ...
        // Fonction globale pour basculer le sélecteur d'entreprise
    ...
    updateActiveMenuItem(action) {
        // Cette fonction pourrait être implémentée pour mettre à jour visuellement l'élément actif
    }
    ...
    validateDataIntegrity() {
        ...
        if (failed !== -1) {
            throw new Error(`Validation des données échouée au contrôle ${failed + 1}`);
        }
    }
    ...
    displayVersionInfo() {
        console.log(`
╔══════════════════════════════════════════════════════════════╗
║                    DOUKÈ Compta Pro v${this.version}                    ║
║              Logiciel de Comptabilité SYSCOHADA             ║
║                                                              ║
║  🏢 Gestion Multi-Entreprises                               ║
║  👥 Gestion des Utilisateurs & Droits                      ║
║  📝 Écritures Comptables SYSCOHADA                         ║
║  🗂️  Plan Comptable SYSCOHADA Révisé                       ║
║  💰 Gestion des Caisses                                    ║
║  📊 Rapports & États Financiers                            ║
║                                                              ║
║  ⚡ Modules chargés: ${Object.keys(this.modules).length.toString().padStart(2)}                                ║
║  🔐 Sécurité: Activée                                      ║
║  🎨 Thème: Dark/Light                                       ║
║                                                              ║
║              Développé par l'équipe DOUKÈ                   ║
╚══════════════════════════════════════════════════════════════╝
        `);
    }
...
// Initialiser le gestionnaire unifié global
...
console.log('🎯 Gestionnaire Unifié Principal chargé - DOUKÈ Compta Pro v3.1');
