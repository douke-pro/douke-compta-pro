...
// Version : V24 — Pool PostgreSQL robuste + initDB avec retry
// =============================================================================
...
const initDB = async (retries = 5, delay = 3000) => {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            // Migration douce : ajouter colonnes manquantes sur bases existantes
            await pool.query(`
                ALTER TABLE financial_reports_requests
                    ADD COLUMN IF NOT EXISTS requested_by_name VARCHAR(150);
            `);

            console.log('✅ [DB] Tables financial_reports initialisées avec succès');
...
app.listen(PORT, async () => {
    ...
    // ET maintient la connexion PostgreSQL active
    // ==========================================================================
    if (process.env.NODE_ENV === 'production' || process.env.RENDER) {
        ...
        setInterval(async () => {
            try {
                // Ping HTTP pour garder le service éveillé
            ...
            try {
                // Ping PostgreSQL pour maintenir la connexion active
                const pool = require('./services/dbService');
                await pool.query('SELECT 1');
