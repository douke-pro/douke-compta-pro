...
;
            ...
            await pool.query(`
                INSERT INTO document_templates (company_id, template_type, template_name, template_html, created_by)
                SELECT 12, 'contrat_cdd', 'Contrat CDD — CCI PARTNERS', $cci_cdd$<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">
<style>
body{font-family:Arial,sans-serif;font-size:11.5pt;color:#1a1a1a;margin:0;padding:20px}
@media print{
...
.partie-block{background:#f5f7fa;border-left:4px solid #1a3a5c;padding:10px 14px;margin:8px 0;border-radius:4px}
.partie-block h3{font-size:10pt;color:#1a3a5c;margin:0 0 5px 0;text-transform:uppercase}
.partie-block p{margin:3px 0;font-size:11pt;line-height:1.5}
.separateur{text-align:center;font-weight:bold;color:#1a3a5c;margin:8px 0;font-size:11pt}
.intro{text-align:center;margin:10px 0;font-size:11pt}
.article{margin:12px 0}
.article h4{font-size:11pt;color:#1a3a5c;text-transform:uppercase;border-bottom:1px solid #dde;padding-bottom:3px;margin-bottom:6px;margin-top:0}
.article p{margin:5px 0;line-height:1.6;text-align:justify;font-size:11pt}
.article ul{margin:5px 0;padding-left:20px}
...
.legal-ref{font-size:8pt;color:#aaa;text-align:center;margin-top:16px;border-top:1px solid #eee;padding-top:6px}
</style></head><body>
...
<!-- PAGE 1 : Parties + Art. 1-3 -->
<div class="no-break">
  <p class="intro"><strong>ENTRE</strong></p>
...
<p class="intro no-break"><strong>Fait à <span class="hl">{{lieu_signature}}</span> en trois exemplaires, le <span class="hl">{{date_signature}}</span></strong></p>
...
<div class="legal-ref">Contrat conforme — Code du Travail Bénin Loi n° 98-004 | CNSS affiliée dès confirmation période d'essai | OHADA</div>
</body></html>$cci_cdd$, NULL
                WHERE NOT EXISTS (
                    SELECT 1 FROM document_templates WHERE company_id = 12 AND template_type = 'contrat_cdd'
                );
            `);
            // CDI CCI PARTNERS
            await pool.query(`
                INSERT INTO document_templates (company_id, template_type, template_name, template_html, created_by)
                SELECT 12, 'contrat_cdi', 'Contrat CDI — CCI PARTNERS', $cci_cdi$<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">
<style>
body{font-family:Arial,sans-serif;font-size:11.5pt;color:#1a1a1a;margin:0;padding:20px}
@media print{
  body{padding:10mm 15mm}
...
<div class="header no-break">
  <h1>{{nom_entreprise}}</h1>
  <h2>CONTRAT DE TRAVAIL À DURÉE INDÉTERMINÉE (CDI)</h2>
  <p>Ref : {{reference_contrat}} &nbsp;|&nbsp; Date : {{date_signature}}</p>
</div>
...
<div class="article no-break">
  ...
  <p>Le présent contrat, conclu pour une durée indéterminée, peut être rompu dans les conditions suivantes :</p>
  <ul>
    <li>Par démission de l'employé, sous réserve du respect du préavis ;</li>
    <li>Par licenciement prononcé par l'employeur pour motif réel et sérieux ;</li>
    <li>Sur accord écrit des deux Parties (rupture conventionnelle) ;</li>
...
<div class="legal-ref">Contrat conforme — Code du Travail Bénin Loi n° 98-004 | CNSS affiliée dès confirmation période d'essai | OHADA</div>
                ...
                WHERE NOT EXISTS (
                    SELECT 1 FROM document_templates WHERE company_id = 12 AND template_type = 'contrat_cdi'
                );
            `);
