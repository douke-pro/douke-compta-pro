...
# --- 2) Nouvelle fonction, inseree juste avant createPayslip ---
...
# ============================================================
# 3) routes/hr.js — route inseree AVANT /payslips/:id (critique)
# ============================================================
with open('routes/hr.js', 'r', encoding='utf-8') as f:
    routes = f.read()

OLD_ROUTES = """router.get('/payslips',              protect, checkCompanyAccess, allowAll,      hrCtrl.listPayslips);
router.get('/payslips/:id',          protect, checkCompanyAccess, allowAll,      hrCtrl.getPayslip);"""

NEW_ROUTES = """router.get('/payslips',              protect, checkCompanyAccess, allowAll,      hrCtrl.listPayslips);
router.get('/payslips/calculate',    protect, checkCompanyAccess, allowEditOnly, hrCtrl.calculatePayslipPreview);
router.get('/payslips/:id',          protect, checkCompanyAccess, allowAll,      hrCtrl.getPayslip);"""

routes = guarded_replace(routes, OLD_ROUTES, NEW_ROUTES, 1, "insertion route calculate avant :id")

with open('routes/hr.js', 'w', encoding='utf-8') as f:
    f.write(routes)

print("OK - routes/hr.js patche")
