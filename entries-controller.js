// =============================================================================
// DOUKÈ Compta Pro - Contrôleur de gestion des écritures v3.1
// =============================================================================
...
class EntriesController {
    constructor(securityManager, dataManager) {
        ...
        console.log('📝 EntriesController initialisé');
    }

    // Charger la page de gestion des écritures
    loadEntriesPage() {
                ...
                <div class="flex justify-between items-center">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
                            ${isCashier ? 'Opérations Caisse' : 'Écritures Comptables'}
                        </h2>
                        <p class="text-gray-600 dark:text-gray-400">${companyName}</p>
                    </div>
                    <div class="flex items-center space-x-4">
                        <div class="text-sm text-gray-500 dark:text-gray-400">
                            ${entries.length} écriture(s) total
                        </div>
                        <button onclick="window.entriesController.showNewEntryModal()" 
                            ...
                            <i class="fas fa-plus mr-2"></i>Nouvelle ${isCashier ? 'opération' : 'écriture'}
                        </button>
                    </div>
                </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border-l-4 border-success">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-bold text-success">${entries.filter(e => e.status === 'Validé').length}</p>
                            </div>
                        ...
                        <div class="mt-2 text-sm text-success">
                            ${((entries.filter(e => e.status === 'Validé').length / entries.length) * 100).toFixed(1)}% du total
                        </div>
                    </div>
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex space-x-2">
                                ...
                                <button onclick="window.entriesController.validateSelectedEntries()" 
                                    ...
                                    <i class="fas fa-check-double mr-1"></i>Valider sélection
                                </button>
                                ` : ''}
    ...
    // Générer le tableau des écritures
        ...
        if (filteredEntries.length === 0) {
            return `
                <div class="text-center py-12 text-gray-500">
                    ...
                    <h3 class="text-lg font-semibold mb-2">Aucune écriture trouvée</h3>
                    <p>Commencez par créer votre première écriture pour cette entreprise.</p>
                    <button onclick="window.entriesController.showNewEntryModal()" 
                        ...
                        ${filteredEntries.map(entry => `
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    ${entry.validatedBy ? `
                                    <div class="text-xs text-gray-500 mt-1">
                                        Validé le ${new Date(entry.validatedAt).toLocaleDateString('fr-FR')}
                                    </div>
                                    ` : ''}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex space-x-2">
                                        <button onclick="viewEntry(${entry.id})" 
                                                class="text-primary hover:text-primary/80" title="Voir détails">
        ...
        const modalContent = `
            <form id="newEntryForm" class="space-y-6">
                ...
                <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                    <h4 class="font-medium text-gray-900 dark:text-white mb-4">Informations générales</h4>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select id="entryJournal" required onchange="window.entriesController.updatePieceNumber()"
                                ...
                                <option value="JG">JG - Journal Général</option>
                                ...
                                <option value="JOD">JOD - Journal des Opérations Diverses</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="mt-4">
                        ...
                        <input type="text" id="entryLibelle" required placeholder="Description de l'opération"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary dark:bg-gray-700 dark:text-white text-base">
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800">
                    ...
                    <div class="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded">
                        <div class="grid grid-cols-3 gap-4 text-sm">
                            ...
                            <div>
                                <span class="text-gray-600 dark:text-gray-400">Équilibre:</span>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-600">
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-check mr-2"></i>Créer l'écriture
                    </button>
                </div>
            </form>
        `;

        window.unifiedManager.modalManager.show('Nouvelle Écriture', modalContent, { size: 'large' });
    ...
    addEntryLine() {
        ...
        newLine.innerHTML = `
            ...
            <div class="col-span-2">
                <label class="block text-xs text-gray-500 mb-1">Débit</label>
                <input type="number" name="debit" min="0" step="0.01" onchange="window.entriesController.updateTotals()"
                       class="w-full px-2 py-1 border border-gray-300 dark:border-gray-600 rounded text-sm focus:ring-1 focus:ring-primary dark:bg-gray-700 dark:text-white">
            </div>
            <div class="col-span-2">
                <label class="block text-xs text-gray-500 mb-1">Crédit</label>
    ...
    // Méthodes utilitaires pour les écritures
    filterEntries() {
    ...
    generateCompanySelectionRequired() {
        return `
            <div class="text-center p-8">
                ...
                <p class="text-gray-600 dark:text-gray-400 mt-2 mb-6">Sélectionnez une entreprise dans la barre latérale pour accéder aux écritures.</p>
...
console.log('📦 Module EntriesController chargé');
