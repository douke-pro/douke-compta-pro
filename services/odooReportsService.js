...
// ✅ FIX 1 : base 10 (pas base 2)
// const ADMIN_UID = parseInt(process.env.ODOO_ADMIN_UID, 10); 
// → On utilise directement ADMIN_UID_INT importé depuis odooService (déjà vérifié)
...
class OdooReportsService {
    ...
    async extractFinancialData(companyId, periodStart, periodEnd, accountingSystem) {
        try {
            ...
            console.log(`[OdooReportsService] Période: ${periodStart} → ${periodEnd} | Système: ${accountingSystem}`);
            ...
            const annexesData        = this.prepareAnnexesData(enrichedLines, companyInfo, accountingSystem, periodStart, periodEnd);
    ...
    // ✅ FIX 2 : odooExecuteKw() au lieu de odoo.execute_kw()
    ...
    calculateCompteResultat(enrichedLines, accountingSystem) {
        ...
        return {
            charges, produits,
            totaux: { charges: totalCharges, produits: totalProduits, resultat, resultat_label: resultat >= 0 ? 'Bénéfice' : 'Perte' }
        };
    }
    ...
    calculateTableauFluxTresorerie(enrichedLines) {
        ...
        const tresoLines = enrichedLines.filter(l => ['51','52','53','54','55','56','57'].some(c => l.account_code.startsWith(c)));
