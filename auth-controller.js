// =============================================================================
// DOUKÈ Compta Pro - Contrôleur d'authentification v3.1
// =============================================================================
...
class AuthController {
    ...
    authenticateUser(email, password) {
        ...
        if (user.status !== 'Actif') {
            return {
                success: false,
                message: 'Compte utilisateur désactivé. Contactez votre administrateur.'
            };
        }
    ...
    resetPassword(userId, newPassword = null) {
        ...
        if (!window.app.currentUser || window.app.currentUser.profile !== 'admin') {
            throw new SecurityError('Seuls les administrateurs peuvent réinitialiser les mots de passe');
        }
    ...
    // Remplir automatiquement les identifiants (pour les comptes de démonstration)
...
function handleSecureLogin() {
    ...
    setTimeout(() => {
        ...
        if (result?.success) {
            showMainApp();
            if (window.unifiedManager?.notificationManager) {
                window.unifiedManager.notificationManager.show('success', 'Connexion réussie', `Bienvenue ${result.user.name} !`);
            }
        } else {
            if (window.unifiedManager?.notificationManager) {
...
function secureLogout() {
    ...
    showLoginInterface();
    if (window.unifiedManager?.notificationManager) {
        window.unifiedManager.notificationManager.show('info', 'Déconnexion', 'Déconnexion sécurisée effectuée. À bientôt !');
    }
}
...
console.log('📦 Module AuthController chargé');
